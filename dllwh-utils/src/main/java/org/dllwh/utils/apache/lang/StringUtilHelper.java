 ...
 *
 * @类描述: 字符串工具类(继承与Apache common中的StringUtils 类)
 * @创建者: 独泪了无痕--duleilewuhen@sina.com
 * @创建时间: 2019年5月7日 下午11:33:49
 * @版本: V1.0.1
 * @since: JDK 1.8
 */
public final class StringUtilHelper extends StringUtils {
	/** 特殊字符串 */
	public static char[] symbols = { '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '_', '+', '=',
			',', '.', ':', ';', '"', '，', '。', '：', '；', ' ', '`', '~', '！', '/', '?', '？', '、', '<', '>',
			'》', '《', '）', '（', '|', 'Θ', '∧', 'Γ', 'α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'ι', 'κ', 'λ', 'μ',
			'ν', 'ξ', 'ο', 'π', 'ρ', 'σ', 'τ', 'υ', 'φ', 'χ', 'ψ', 'ω', 'Г', 'Д', 'Ё', 'Ж', 'З', 'И', 'Й',
			'Л', 'Ф', 'Ц', 'Ч', 'Ш', 'Щ', 'Ъ', 'Ы', 'Ь', 'Э', 'Ю', 'Я', 'б', 'г', 'д', 'ё', 'ж', 'з', 'и',
			'й', 'к', 'ā', 'á', 'ǎ', 'à', 'ō', 'ó', 'ǒ', 'ò', 'ê', 'ē', 'é', 'ě', 'è', 'ī', 'í', 'ǐ', 'ì',
			'ū', 'ú', 'ǔ', 'ù', 'ǖ', 'ǘ', 'ǚ', 'ǜ', '§', '№', '☆', '★', '○', '●', '◎', '◇', '◆', '□', '■',
			'△', '▲', '※', '→', '←', '↑', '↓', '〓', '＃', '＆', '＠', '⊙', '⓪', '①', '②', '③', '④', '⑤', '⑥',
			'⑦', '⑧', '⑨', '⑩', '⑪', '⑫', '⑬', '⑭', '⑮', '⑯', '⑰', '⑱', '⑲', '⑳', '⊕', '¤', '㊣', '▂', '▃',
			'▄', '▅', '▆', '▇', '█', '▓', '回', '≡', '╝', '╚', '╔', '╗', '╬', '╓', '╩', '┠', '┨', '┯', '┷',
			...
			'が', 'き', 'ぎ', 'く', 'ぐ', 'け', 'げ', 'こ', 'ご', 'さ', 'ざ', 'し', 'じ', 'す', 'ず', 'せ', 'ぜ', 'そ', 'ぞ',
			'た', 'だ', 'ち', 'ぢ', 'っ', 'つ', 'づ', 'て', 'で', 'と', 'ど', 'な', 'に', 'ぬ', 'ね', 'の', 'は', 'ば', 'ぱ',
			'ひ', 'び', 'ぴ', 'ふ', 'ぶ', 'ぷ', 'へ', 'べ', 'ぺ', 'ほ', 'ぼ', 'ぽ', 'ま', 'み', 'む', 'め', 'も', 'ゃ', 'や',
			'ゅ', 'ゆ', 'ょ', 'よ', 'ら', 'り', 'る', 'れ', 'ろ', 'ゎ', 'わ', 'ゐ', 'ゑ', 'を', 'ん', 'ゔ', 'ゕ', 'ゖ', 'ァ',
			...
			'ヮ', 'ワ', 'ヰ', 'ヱ', 'ヲ', 'ン', 'ヴ', 'ヵ', 'ヶ', 'ヷ', 'ヸ', 'ヹ', 'ヺ', '・', 'ー', 'ヽ', 'ヾ', 'ヿ', '゠',
			'ㇰ', 'ㇱ', 'ㇲ', 'ㇳ', 'ㇴ', 'ㇵ', 'ㇶ', 'ㇷ', 'ㇸ', 'ㇹ', 'ㇺ', 'ㇻ', 'ㇼ', 'ㇽ', 'ㇾ', 'ㇿ', '零', '壹', '贰',
			'叁', '肆', '伍', '陆', '柒', '捌', '玖', '拾', '佰', '仟', '万', '亿', '吉', '太', '拍', '艾', '分', '厘', '毫',
			'微', '卍', '卐', '卄', '巜', '弍', '弎', '弐', '朤', '氺', '曱', '甴', '囍', '兀', '々', '〆', '〡', '〢', '〣',
			'〤', '〥', '〦', '〧', '〨', '〩', '㊎', '㊍', '㊌', '㊋', '㊏', '㊚', '㊛', '㊐', '㊊', '㊤', '㊥', '㊦', '㊧',
			'㊨', '㊒', '㊫', '㊑', '㊓', '㊔', '㊕', '㊖', '㊗', '㊘', '㊜', '㊝', '㊞', '㊟', '㊠', '㊡', '㊢', '㊩', '㊪',
			'㊬', '㊭', '㊮', '㊯', '㊰', '㊀', '㊁', '㊂', '㊃', '㊄', '㊅', '㊆', '㊇', '㊈', '㊉', '♚', '♛', '♝', '♞',
			'♜', '♟', '♔', '♕', '♗', '♘', '♖', 'ㄅ', 'ㄆ', 'ㄇ', 'ㄈ', 'ㄉ', 'ㄊ', 'ㄋ', 'ㄌ', 'ㄍ', 'ㄎ', 'ㄏ', 'ㄐ',
			'ㄑ', 'ㄒ', 'ㄓ', 'ㄔ', 'ㄕ', 'ㄖ', 'ㄗ', 'ㄘ', 'ㄙ', 'ㄚ', 'ㄛ', 'ㄜ', 'ㄝ', 'ㄞ', 'ㄟ', 'ㄠ', 'ㄡ', 'ㄢ', 'ㄣ',
			'ㄤ', 'ㄥ', 'ㄦ', 'ㄧ', 'ㄨ', 'ㄩ', '☀', '☁', '☂', '☃', '☄', '☇', '☈', '☊', '☋', '☌', '☍', '♫', '♬',
			'♩', '♭', '♪', '∷', '﹌', '°', '′', '″', '＄', '￥', '〒', '￠', '￡', '％', '℃', '℉', '﹩', '﹪', '‰',
			'﹫', '㎡', '㏕', '㎜', '㎝', '㎞', '㏎', '㎏', '㎎', '㏄', 'º', '¹', '²', '³', '☏', '✐', '✎', '✏', '✑',
			'✒', '✍', '✉', '✁', '✂', '✃', '✄', '✆', '☎', '☑', '✓', '✔', '☐', '☒', '✗', '✘', '✕', '✖', '☢',
